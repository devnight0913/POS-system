...
@section('content')
    ...
    <x-card>
        ...
        <form action="{{ route('payments.update', $payment) }}" method="POST" role="form">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select name="customer" id="customer" class=" form-select @error('customer') is-invalid @enderror">
                    ...
                    @foreach ($customers as $customer)
                        <option value="{{ $customer->id }}" @selected($payment->customer_id == $customer->id)>{{ $customer->name }} - {{ $customer->number }}
                        </option>
                    @endforeach
                </select>
                @error('customer')
            ...
            <div class="mb-3">
                ...
                <select name="employee" id="employee" class=" form-select @error('employee') is-invalid @enderror">
                    ...
                    @foreach ($employees as $employee)
                        <option value="{{ $employee->id }}" @selected($payment->employee_id == $employee->id)>{{ $employee->name }} - {{ $employee->price }}
                        </option>
                    @endforeach
                </select>
                @error('employee')
